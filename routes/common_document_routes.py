...
def download_document(document_id):
    ...
    try:
        ...
        if not file_path or not os.path.exists(file_path):
            # Fallback: check if content is stored in DB (legacy or small files)
            # This logic mimics attachment_routes if you store base64, but here we used file storage above.
            return jsonify({'success': False, 'message': 'File not found on server'}), 404
            
        return send_file(
            file_path,
            as_attachment=True,
            download_name=doc.get('file_name', 'document')
        )
